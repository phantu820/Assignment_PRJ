...
public class UpdateServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (category.isEmpty()) category = current.getCategory();
        if (sp.isEmpty()) sp = current.getSpecialized();
        if (author.isEmpty()) author = current.getAuthor();
        if (publis.isEmpty()) k = current.getPublishingYear();
        else if (!isNumeric(publis)) {
        request.setAttribute("ms", "Public Year is not Invalid!!");
        request.getRequestDispatcher("updateBook.jsp").include(request, response);
        request.getRequestDispatcher("home_management.jsp").forward(request, response);
        }
        else if (publis.isEmpty()) k = current.getPublishingYear();

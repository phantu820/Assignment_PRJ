   ...
   List<Book> listBook = new ArrayList<>();
    ...
    throws ServletException, IOException {
      ...
      int k = Integer.parseInt(page);
       if (listBook.size() <= (k-1)*10 )
                request.setAttribute("listBooks", null);
            else if (listBook.size() <= (k)*10 ) 
                request.setAttribute("listBooks", listBook.subList((k-1)*10, listBook.size()));
            else
                request.setAttribute("listBooks", listBook.subList((k-1)*10, k*10));
    ...
    throws ServletException, IOException {
       ...
       if (ls.size() <= (k-1)*10 )
                request.setAttribute("listBooks", null);
            else if (ls.size() <= (k)*10 ) 
                request.setAttribute("listBooks", ls.subList((k-1)*10, ls.size()));
            else
                request.setAttribute("listBooks", ls.subList((k-1)*10, k*10));
        request.getRequestDispatcher("SearchList.jsp").include(request, response);
        request.setAttribute("isSearch", 0);
        if (a.getRole() == 0)
        request.getRequestDispatcher("home_user.jsp").forward(request, response);

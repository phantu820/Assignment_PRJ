...
public class BooksDAO extends MyDAO {
  ...
  public void insert(Book x) {
    ...
    public void delete(String ID) {
     ...
     xSql = "update BOOKS set Name=?, Category=?,specialized=?,author=?,PublishingYear=? where ID=?";
     try {      
        ps = con.prepareStatement(xSql);
        ps.setString(1, x.getName());
        ps.setString(2, x.getCategory());
        ps.setString(3, x.getSpecialized());
        ps.setString(4, x.getAuthor());
        ps.setInt(5, x.getPublishingYear());
        ps.setString(6, ID);
        ps.executeUpdate();
        ps.close();
     }
